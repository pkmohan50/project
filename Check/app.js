		...
		var flyRef = database.ref('Flights').orderByChild('Time').startAt('13:50');
		...
		function gotData(data){
			...
			for(var i = 0; i < keys.length; i++) {
				...
				document
					...
					.innerHTML+='<li class="list-group-item Flights[k]" style="border-style:solid; text-align:center; border-color:Teal; border-radius:50px; border-width:10px; font-size:30px; margin:20px; padding:10px 25px 10px 25px;  box-shadow: 0 0 20px 0 rgba(0, 0, 0, 0.2), 0 5px 5px 0 rgba(0, 0, 0, 0.24); text-align:center; position:relative; top:20px"<div  class="lead"><img style="position:relative; display:block; top:90px; text-align:right;" src="https://s15.postimg.org/aq8a956t7/462084.jpg" width="150" height="150"><p style="position:relative; bottom:75px; ">'+Flights[k].Airlines+'</p></div> <p style="position:relative; bottom:75px;">'+Flights[k].Type+'</p><p style="position:relative; bottom:75px;">'+Flights[k].Status+'</p></li>';
				//var li=document.createElement('li',Airlines+":"+Type);
				//$(li).parent('flightlist');
				//documents.getElementById("type").innerHTML=Type;
			}
		}
